package dev.vk.jfc.jfccommon;

public abstract class Jfc {
    public static final String K_HOSTNAME = "hostname";
    public static final String K_SOURCE = "source";
    public static final String K_TIMESTAMP = "timestamp";
    public static final String K_BROKER_TIMESTAMP = "brokerTimestamp";
    public static final String K_FRAMENO = "frameNo";
    public static final String K_LOCALID = "localID";
    public static final String K_FRAME_STORAGE_PATH = "frameStoragePath";
    public static final String K_UUID = "uuid";
    public static final String K_PARENT_UUID = "parentUuid";
    public static final String K_S3PATH = "s3Path";
    public static final String K_MESSAGE_TYPE = "message-type";

    public static final String TP_PROCESSED_FRAME_FACES = "processed-frame-faces";
    public static final String TP_PROCESSED_FRAME_FACE = "processed-frame-face";
    public static final String TP_PROCESSED_FRAME = "processed-frame";
}
